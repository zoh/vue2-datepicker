<template>
  ...
  >
    <div v-if="!inline" :class="`${prefixClass}-input-wrapper`" @mousedown="openPopup">
      ...
      <i v-else :class="`${prefixClass}-icon-calendar`">
      ...
      <div :class="`${prefixClass}-datepicker-content`">
        ...
        <div v-if="hasSlot('footer') || confirm" :class="`${prefixClass}-datepicker-footer`">
          <slot
            name="footer"
            :value="currentValue"
            :isEmptyCurValue="isEmptyCurValue"
            :emit="emitValue"
          ></slot>
          <button
            v-if="confirm"
...
export default {
  ...
  computed: {
    ...
    isEmptyCurValue() {
      return Array.from(this.currentValue).filter(val => val && isValidDate(val)).length === 0;
    },
  },
  watch: {
    innerValue: {
  ...
  mounted() {
    if (this.range)
      this.$on('calendar-range-select', value => {
        const to = value[1]; // 0 - first
        if (this.currentValue.length > 1 && (!to || !isValidDate(to))) {
          this.currentValue = [];
  ...
  methods: {
    ...
    handleClear() {
      this.emitValue(this.range ? [] : null);
      this.$emit('clear');
    },
    handleConfirmDate() {
